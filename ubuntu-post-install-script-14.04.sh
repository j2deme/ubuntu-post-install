...
function favourites {
  ...
  [Yy]* )
    ...
    sudo add-apt-repository -y ppa:shutter/ppaw
    sudo apt-get update -qq
    sudo apt-get install -y --no-install-recommends easytag filezilla gnome-tweak-tool nautilus-dropbox nautilus-open-terminal pyrenamer vlc gparted pgadmin3 wajig tasksel mysql-workbench dia guake diffuse shutter cheese meld
    echo 'Done.'
    main
    ;;
  # Negative action
...
function system {
  ...
  echo '3. Install Oracle Java 8 (JDK & JRE)'
  echo 'r. Return.'
  echo '---'
  ...
  2)
    ...
    * )
    ...
    ;;
  3)
    echo 'Adding WebUp8Team/Java PPA to software sources...'
    echo 'Requires root privileges:'
    sudo add-apt-repository -y ppa:webupd8team/java
    echo 'Updating repository information...'
    sudo apt-get update -qq
    echo 'Installing JDK8 and JRE8...'
...
function development {
  ...
  1)
    ...
    [Yy]* )
      echo 'Requires root privileges:'
      # Feel free to change to whatever suits your preferences.
      sudo apt-get install -y devscripts gcc-snapshot gnome-common git gitg giggle python-launchpadlib python3-distutils-extra build-essential libssl-dev curl g++
      echo 'Done.'
      development
      ;;
    # Negative action
...
function androidstudio {
  echo 'Adding PaoloRotolo/Android-Studio PPA to software sources...'
  echo 'Requires root privileges:'
...
function thirdparty {
  ...
  echo '4. Steam?'
  echo '5. Sublime Text 3 & Sublime Package Control?'
  echo '6. Spotify client'
  echo '7. Install Unity Tweak Tool?'
  echo '8. Install My Weather Indicator?'
  echo '9. Install Variety Wallpaper Changer?'
  echo '10. Install Android Studio'
  echo 'r. Return'
  echo ''
  read -p 'Enter your choice: ' REPLY
  case $REPLY in
  ...
  10)
    ...
    ;;
  11)
    echo 'Adding Atom Text Editor repository to sources...'
